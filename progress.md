...
### ★★ 교수 2차 피드백 → 방향 재정립 (2026-06-28)
- 교수 #2 답변 도착(전문 `docs/2026-06-28-professor-feedback-q2.md`). **재프레이밍:** ①공개 데이터셋=전부 알려진 공격(제로데이 아님), '미관측'은 '학습분할 부재'=규칙 정교화 평가 ②알려진 공격 ~100% 탐지가 기본 → 봇넷 0.49는 천장 아님(행위 분석 정교화 부족) ③행위 기반·시나리오·연관분석·오픈규칙(Snort/Suricata) 방향 ④자원 효율 부수적 ⑤위험도 가볍게+반복. 교수 "소논문 가능" 언급.
- 'zero-day/전이 본질적 천장' 표현 폐기. 메모리 `wemeet-professor-feedback` 추가, findings·overview·project-redefinition 정정.
- 심층 방향 워크플로우(wf_8639c50d) **완료**(에이전트 8·45만토큰): 6각도 리서치→종합→적대검증. **확정안 `docs/2026-06-28-bot-detection-direction.md`** (판정: 조건부 GO).
- **검증 핵심 발견:** ①현재 0.49 FP=in-sample 누수(교체 필요) ②temporal이 port_key groupby + Bot 98.4%=8080 + temp_imp 0.84 → **'8080 지문' 위험**(그룹핑키 ablation 필수) ③타임스탬프 초단위 → FFT 주기성 불가(robust di 통계만) ④DistriNet Improved IP 재수록은 미확인 가정(Phase0 헤더 확인). → **4주 목표를 'recall↑'에서 '행위 증명+FP교정+정직 프레이밍'으로 재배분.**
