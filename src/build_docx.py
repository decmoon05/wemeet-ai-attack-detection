...
def build_q2(out=None):
    ...
    P(d, "계획만 세우다가 실제로 만들고 평가해보니 생각보다 막히는 부분이 많았습니다. 저희가 부딪힌 어려움과, 멘토님께 여쭙고 싶은 점을 정리했습니다.", size=10.5)
    ...
    items = [
        ...
        ("5. 어디까지 만드는 게 적당한지 판단이 어렵습니다",
         ["AI 도구의 도움으로 개발 속도는 빨라졌지만, '이 프로젝트를 어디까지 만드는 게 적당한가'를 정하기가 어렵습니다. 지금은 네트워크 탐지 부분과 보안 로그 분석 보조 기능까지 만들어 1차로 마무리한 상태입니다.",
          "여쭙고 싶은 점 — 학부 산학 프로젝트로서, '완성도 높은 탐지기를 만드는 것'보다 '정직하게 평가하고 한계를 분명히 밝히는 것'에 무게를 두는 지금 방향이 괜찮은지 의견을 듣고 싶습니다."]),
    ]
